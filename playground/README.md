# playground — 9기법(+번외 4) case별 체험 키트

이론편 슬라이드의 **각 사건(case)을 직접 손으로 돌려보기 위한** 개인 체험용 워크스페이스입니다.
case10~13(검증·리뷰)은 v1.0에서 세미나 범위에서 제외된 사건이지만, 동작하는 키트라 **번외**로 남겨 두었습니다.
...
> ⚠️ **세미나의 실습 세션은 없습니다.** 이 폴더는 발표자가 혼자 기법을 체감해 보기 위해 만든
> 비공식 부속물이며, 세미나 후 각자 손으로 돌려보는 용도로 공개해 둡니다.
...
7. 일부 before 스킬(②④⑫)에는 "직접 읽고 직접 센다 · 스크립트 금지" 같은 **재현 장치**가 들어 있다.
   요즘 모델은 시키지 않아도 알아서 코드·검색으로 우회하는데, 그 우회가 바로 해당 기법이기 때문이다.
...
### 번외 · 검증·리뷰 — "'됐습니다'를 '증명됐습니다'로" (세미나 범위 밖)
