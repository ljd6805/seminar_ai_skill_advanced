...
```
`analysis.bad.md` 는 재현 절차 빈칸 + 영향 범위 TODO → **진행 거부**.
`analysis.good.md` 는 세 섹션이 채워짐 → **PASS**.

## 스킬로 체험
1. **Before** — `before/` incident-report: analysis.md 를 검사 없이 신뢰하고 시작.
   `output/analysis.bad.md 로 장애 리포트 써줘` → 빈 분석을 그대로 근거로 삼거나 지어낸다.
2. **After** — `after/` incident-report: 도입부에서 gate.py 를 먼저 실행.
   - bad → "GATE FAIL — 재현 절차/영향 범위. 분석 보완 후 재시도" 로 **멈춘다**.
   - good → PASS 후 리포트 작성, 끝에 PASS 로그 첨부.
