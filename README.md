...
- **이론편 — 기법 카탈로그** (90분 목표): **14개의 사건**을 사건당 네 장으로 푼다 — **문제 장**(문제 예시 재현 → 문제의 스킬 원문 → 원인 분석), **해결 장**(해결 리플레이 → 해결된 스킬 발췌 → 기술소개), **원리 장**(기법의 작동 원리를 움직이는 다이어그램 한 장으로 — 스킬이 처음인 청중용), **개조 명세 장**(Before/After 스킬 전문 대조 — 스킬북의 본편). 기법의 이름은 항상 답으로서 마지막에 등장한다.
  ...
  - **PART 2 · 스킬 연계술** ⑥산출물 계약 ⑦파이프라인 스킬 ⑧라우터 스킬 ⑨게이트 체크 — 하나의 장애 대응 프로세스를 네 사건으로 이어 완성하는 연작. 네 기법은 스킬 **사이**의 네 질문(무엇을 주고받나·순서는·누가 나서나·믿어도 되나)에 답한다 (+ 사이의 네 질문 정리 · 결합 데모 시네마)
...
| [`slides/index.html`](slides/index.html) | 이론편 슬라이드 v0.4.1 (77장, 단일 흐름) — reveal.js 자립형(오프라인 동작), 1·2·3편과 동일한 테마 상속(Wanted Sans Variable + JetBrains Mono 임베드). 사건마다 문제·해결 리플레이 애니메이션, 원인 차트, **원리 도해 애니메이션**(서랍장·검문 게이트·창 분할 등 사건별 은유) 포함 |
...
- [x] 시놉시스 v0.3
- [x] 이론편 슬라이드 v0.4.1 (77장 — 2편 5이론 복습 · 사건 14건 × [문제·해결·원리 도해·개조 명세], PART 2 "사이의 네 질문" 정리, 256k 가계부·파이프라인 시네마·결함 낙하 그물 애니메이션, 선택 지도·종합 사례·부록 요약표)
- [ ] 슬라이드 검토·보강 ← **다음 단계**
- [ ] 리허설 및 시간 조정
