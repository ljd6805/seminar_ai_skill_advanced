...
AI Agent 팀 세미나 시리즈 **3편 — SKILL 심화** 의 발표 자료 저장소입니다.

> **핵심 메시지**: 스킬 하나는 절차를 담고, 스킬 시스템은 한계를 설계한다 — **202k/262k는 벽이 아니라 설계 조건이다.**
...
- **이론편 — 기법 카탈로그** (90분 목표): **9개의 사건**을 사건당 네 장으로 푼다 — **문제 장**(문제 예시 재현 → 문제의 스킬 원문 → 원인 분석), **해결 장**(해결 리플레이 → 해결된 스킬 발췌 → 기술소개), **원리 장**(기법의 작동 원리를 움직이는 다이어그램 한 장으로 — 스킬이 처음인 청중용), **Before/After 장**(고치기 전·후의 SKILL.md 전문 대조 — 스킬북의 본편). 기법의 이름은 항상 답으로서 마지막에 등장한다.
  - **PART 1 · 컨텍스트 절약술** ①레퍼런스 계층화 ②스크립트 위임 ③외부 기억 ④지도 우선 탐색 ⑤분업 요약 — 사내 LLM의 **컨텍스트 제약(MAX 202k · PRO 262k)**을 설계 조건으로 삼는 다섯 사건
...
- [x] 시놉시스 v1.0
- [x] 이론편 슬라이드 v1.0 (54장 — 2편 5이론 복습 · 사건 9건 × [문제·해결·원리 도해·Before/After], PART 2 "연결의 네 질문" 정리, "하나의 흐름"(부품→라인) 파트 연계 정리, 컨텍스트 가계부·파이프라인 시네마 애니메이션, 선택 지도·종합 사례·부록 요약표)
...
54장·9기법·Before/After 전문을 유지한다. 리플레이와 수치 게이지는 교육용 예시이며 성능 실측 결과가 아니다. 사내 LLM 컨텍스트(MAX 202k · PRO 262k)는 이 자료의 환경 가정이며, 예시 산술은 더 작은 202k를 기준으로 한다. SKILL은 절차를 안내하며 강제 실행·차단은 별도 실행기나 훅이 담당한다.
